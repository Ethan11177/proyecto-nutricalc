...
const EditDoctorForm = () => {
    ...
    useEffect(() => {
        ...
        const fetchDoctorData = async () => {
            try {
                const response = await axios.get('http://172.206.238.50:5000/api/obtener_doctor', { params: { correo: email } });
    ...
    const handleSubmit = async (e) => {
        ...
        if (validate() && isPasswordMatch) {
            try {
                const response = await axios.put('http://172.206.238.50:5000/api/editar_doctor', { ...formData, correo: email });
