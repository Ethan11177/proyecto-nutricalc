...
const NewUserForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (validate() && isPasswordMatch) {
            try {
                const response = await axios.post('http://172.206.238.50:5000/api/guardar_doctor', formData);
                console.log(response.data);
                setSnackbarMessage('Registro guardado con éxito');
                setSnackbarSeverity('success');
                setOpenSnackbar(true);
                navigate('/login'); // Redireccionar a la página de inicio después de registrar el usuario
            } catch (error) {
