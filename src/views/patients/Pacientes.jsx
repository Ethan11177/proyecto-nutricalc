...
const Pacientes = () => {
    ...
    const fetchRecords = async (query) => {
        try {
            const response = await axios.get(`http://172.206.238.50:5000/obtener`, {
                params: { query, email }
            });
    ...
    const handleConfirmDelete = async () => {
        try {
            await axios.delete(`http://172.206.238.50:5000/delete/${selectedCorreo}`);
    ...
    const handleInfoPerson = async (correo) => {
        try {
            const response = await axios.get(`http://172.206.238.50:5000/obtener-paciente/${correo}`);
    ...
    return (
        <>
            ...
            >
                <DialogTitle id="info-dialog-title">{"Información de la Dieta"}</DialogTitle>
                <DialogContent>
                    {dietaData ? (
